...
def evaluate_domain_adaptation( models, test_loader, cfg,
    ...
    interp = None
    if fixed_test_size:
        interp = nn.Upsample(size=(cfg.TEST.OUTPUT_SIZE_TARGET, cfg.TEST.OUTPUT_SIZE_TARGET), mode='bilinear', align_corners=True)
    # eval
        ...
        if i_iter not in all_res.keys():
            ...
            for index, batch in tqdm(enumerate(test_loader)):
                ...
                if not fixed_test_size:
                    interp = nn.Upsample(size=(label.shape[1], label.shape[2]), mode='bilinear', align_corners=True)
                with torch.no_grad():
                    pred_main = models[0](image.cuda(device))[1] #取出model的主要輸出
                    output = interp(pred_main).cpu().data[0].numpy() # 取出batch軸的第一個樣本
                    output = output.transpose(1, 2, 0)
                    output = np.argmax(output, axis=2)
                label = label.numpy()[0][0] #label也有4軸，取出batch軸的第一個樣本
...
def load_checkpoint_for_evaluation(model, checkpoint, device):
    """load weight of model and switch state of model to eval mode"""
